...
WHERE username = i_username AND `password` = md5(i_password)) THEN
...
DELIMITER //
CREATE PROCEDURE ad_add_building_tag(IN i_buildingName VARCHAR(55), IN i_tag
VARCHAR(55))
BEGIN
...
DELIMITER //
CREATE PROCEDURE ad_remove_building_tag(IN i_buildingName VARCHAR(55), IN i_tag
VARCHAR(55))
BEGIN
...
DELIMITER //
CREATE PROCEDURE ad_create_building(IN i_buildingName VARCHAR(55), IN i_description
TEXT)
BEGIN
...
DELIMITER //
CREATE PROCEDURE ad_update_building(IN i_oldBuildingName VARCHAR(55), IN
i_newBuildingName VARCHAR(55), IN i_description TEXT)
BEGIN
   UPDATE Building
...
DELIMITER //
CREATE PROCEDURE ad_create_station(IN i_stationName VARCHAR(100), IN 
i_buildingName VARCHAR(100), IN i_capacity INT)
BEGIN
...
BEGIN
   ...
   CREATE TABLE ad_view_station_result(stationName varchar(100), capacity int,
...
DELIMITER //
CREATE PROCEDURE ad_update_station(IN i_stationName VARCHAR(100), IN i_capacity
INT, IN i_buildingName VARCHAR(100))
BEGIN
   UPDATE Station
...
END //
DELIMITER ;
-- Query #17: mn_filter_foodTruck [Screen #11 Manager Manage Food Truck] COMPLETE
...
END //
DELIMITER ;
-- Query #19a: mn_create_foodTruck_add_station [Screen #12 Manager Create Food Truck] COMPLETE
DROP PROCEDURE IF EXISTS mn_create_foodTruck_add_station;
DELIMITER //
CREATE PROCEDURE mn_create_foodTruck_add_station(IN i_foodTruckName VARCHAR(50), IN
i_stationName VARCHAR(50), IN i_managerUsername VARCHAR(50))
BEGIN
INSERT INTO FoodTruck(foodTruckName, stationName, managerUsername)
VALUES (i_foodTruckName, i_stationName, i_managerUsername);
END //
DELIMITER ;
-- Query #19b: mn_create_foodTruck_add_staff [Screen #12 Manager Create Food Truck] COMPLETE
...
DELIMITER //
CREATE PROCEDURE mn_create_foodTruck_add_MenuItem(IN i_foodTruckName VARCHAR(50),
IN i_price DECIMAL(6,2), IN i_foodName VARCHAR(50))
BEGIN
   INSERT INTO MenuItem(price, foodTruckName, foodName)
   VALUES (i_price, i_foodTruckName, i_foodName);
END //
DELIMITER ;
-- Query #20a: mn_view_foodTruck_available_staff [Screen #13 Manager Update Food Truck] COMPLETE
...
WHERE
STAFF.foodTruckName IS NULL;
 
 
END //
DELIMITER ;
-- Query #20b: mn_view_foodTruck_staff [Screen #13 Manager Update Food Truck]
-- Should be shown as FirstName LastName   COMPLETE
...
DELIMITER //
CREATE PROCEDURE mn_update_foodTruck_station(IN i_foodTruckName VARCHAR(50), IN
i_stationName VARCHAR(50))
BEGIN
   UPDATE FoodTruck
...
DELIMITER //
CREATE PROCEDURE mn_update_foodTruck_staff(IN i_foodTruckName VARCHAR(50), IN
i_staffName VARCHAR(50))
BEGIN
   UPDATE STAFF
...
DROP PROCEDURE IF EXISTS mn_update_foodTruck_MenuItem;
DELIMITER //
...
CREATE PROCEDURE mn_filter_summary(
   ...
   IN i_minDate DATE,
   IN i_maxDate DATE,
   IN i_sortedBy ENUM('foodTruckName', 'totalOrder', 'totalRevenue',
'totalCustomer'),
   IN i_sortedDirection ENUM('ASC', 'DESC'))
BEGIN
   DROP TABLE IF EXISTS mn_filter_summary_result;
   CREATE TABLE mn_filter_summary_result(foodTruckName varchar(100), totalOrder
int, totalRevenue decimal(6, 2), totalCustomer int);
   INSERT INTO mn_filter_summary_result
   SELECT foodTruckName, totalOrder, totalRevenue, totalCustomer
   FROM
   (   SELECT FoodTruck.foodTruckName as foodTruckName, COUNT(DISTINCT
(Orders.orderID)) as totalOrder, SUM(MenuItem.price * OrderDetail.purchaseQuantity)
as totalRevenue, COUNT(DISTINCT customerUsername) as totalCustomer
       FROM Orders
       INNER JOIN
...
OrderDetail.foodTruckName = MenuItem.foodTruckName)
       ...
       (i_foodTruckName IS NULL OR FoodTruck.foodTruckName LIKE CONCAT('%',
i_foodTruckName, '%') ) AND
       (i_stationName IS NULL OR FoodTruck.stationName LIKE CONCAT('%',
i_stationName, '%') ) AND
       (i_minDate IS NULL OR Orders.date >= i_minDate) AND (i_maxDate IS NULL OR
Orders.date <= i_maxDate)
       GROUP BY FoodTruck.foodTruckName
   ) T
   ORDER BY
       CASE WHEN (i_sortedBy = NULL AND (i_sortedDirection = "ASC" OR
i_sortedDirection = NULL)) THEN foodTruckName END ASC,
       CASE WHEN (i_sortedBy = NULL AND i_sortedDirection = "DESC") THEN
foodTruckName END DESC,
       CASE WHEN i_sortedBy = "foodTruckName" AND (i_sortedDirection = "ASC" OR
i_sortedDirection = NULL) THEN foodTruckName END ASC,
       CASE WHEN i_sortedBy = "foodTruckName" AND i_sortedDirection = "DESC" THEN
foodTruckName END DESC,
       CASE WHEN i_sortedBy = "totalOrder" AND (i_sortedDirection = "ASC" OR
i_sortedDirection = NULL) THEN totalOrder END ASC,
       CASE WHEN i_sortedBy = "totalOrder" AND i_sortedDirection = "DESC" THEN
totalOrder END DESC,
       CASE WHEN i_sortedBy = "totalRevenue" AND (i_sortedDirection = "ASC" OR
i_sortedDirection = NULL) THEN totalRevenue END ASC,
       CASE WHEN i_sortedBy = "totalRevenue" AND i_sortedDirection = "DESC" THEN
totalRevenue END DESC,
       CASE WHEN i_sortedBy = "totalCustomer" AND (i_sortedDirection = "ASC" OR
i_sortedDirection = NULL) THEN totalCustomer END ASC,
       CASE WHEN i_sortedBy = "totalCustomer" AND i_sortedDirection = "DESC" THEN
totalCustomer END DESC
       ;
END //
DELIMITER ;
...
DELIMITER //
CREATE PROCEDURE mn_summary_detail(IN i_managerUsername VARCHAR(50), IN
i_foodTruckName VARCHAR(50))
BEGIN
   DROP TABLE IF EXISTS mn_summary_detail_result;
   CREATE TABLE mn_summary_detail_result(`date` date, customerName varchar(100),
totalPurchase decimal(6, 2),
       orderCount int, foodNames text);
   INSERT INTO mn_summary_detail_result
   SELECT Orders.date AS `date`, CONCAT(firstName , ' ' , lastName) AS
customerName, SUM(price * purchaseQuantity) AS totalPurchase, COUNT(DISTINCT
(Orders.orderID)) AS orderCount,  GROUP_CONCAT(DISTINCT(OrderDetail.foodName)
SEPARATOR ', ') AS foodNames
       FROM User
       INNER JOIN 
...
DELIMITER //
CREATE PROCEDURE cus_filter_explore(IN i_buildingName VARCHAR(50), IN i_stationName
VARCHAR(50), IN i_buildingTag VARCHAR(50), IN i_foodTruckName VARCHAR(50), IN
i_foodName VARCHAR(50))
BEGIN
   DROP TABLE IF EXISTS cus_filter_explore_result;
   CREATE TABLE cus_filter_explore_result(stationName varchar(100), buildingName
varchar(100),
       foodTruckNames text, foodNames text);
   INSERT INTO cus_filter_explore_result
   SELECT Station.stationName, Station.buildingName,
...
GROUP_CONCAT(DISTINCT(MenuItem.foodName) SEPARATOR ',') AS foodNames
       ...
       (i_buildingTag is NULL OR BuildingTag.tag LIKE CONCAT('%', i_buildingTag,
'%')) AND
       (i_foodTruckName is NULL OR FoodTruck.foodTruckName LIKE CONCAT('%',
i_foodTruckName, '%')) AND
...
DELIMITER //
CREATE PROCEDURE cus_select_location( IN i_customerUsername VARCHAR(50), IN
i_stationName VARCHAR(50))
BEGIN
...
DELIMITER //
CREATE PROCEDURE cus_current_information_foodTruck(IN i_customerUsername
VARCHAR(55))
BEGIN
   DROP TABLE IF EXISTS cus_current_information_foodTruck_result;
   CREATE TABLE cus_current_information_foodTruck_result(foodTruckName
varchar(100), managerName varchar(100), foodNames text);
...
-- afford the food price × quantity. If successful, make sure to deduct
-- from their balance too.
DROP PROCEDURE IF EXISTS cus_add_item_to_order;
DELIMITER //
CREATE PROCEDURE cus_add_item_to_order(IN i_foodTruckName VARCHAR(55), IN
i_foodName VARCHAR(55), IN i_purchaseQuantity INT, IN i_orderId INT)
BEGIN
   SET @foodPrice = (SELECT price FROM MenuItem WHERE foodName = i_foodName AND foodTruckName = i_foodTruckName);
   SET @customerUsername = (SELECT customerUsername FROM Orders WHERE orderID = i_orderId);
   SET @balance = (SELECT balance FROM Customer WHERE Customer.username = @customerUsername);
   IF @foodPrice * i_purchaseQuantity <= @balance THEN
      INSERT INTO OrderDetail VALUES (i_orderId, i_foodTruckName, i_foodName, i_purchaseQuantity);
      UPDATE Customer SET balance = @balance - @foodPrice WHERE Customer.username = @customerUsername;
   END IF;
END //
DELIMITER ;  
...
DROP TABLE IF EXISTS cus_order_history_result;
   CREATE TABLE cus_order_history_result(`date` date, orderID char(10), orderTotal
decimal(6, 2),
foodNames varchar(100), foodQuantity int);
 
INSERT INTO cus_order_history_result SELECT Orders.date AS 'Date', Orders.orderID AS 'OrderID', 
    SUM(OrderDetail.purchaseQuantity * MenuItem.price) AS 'Order Total', 
    GROUP_CONCAT(DISTINCT(OrderDetail.foodName)) AS 'Food(s)',
    SUM(OrderDetail.purchaseQuantity) AS 'Food Quantity' FROM Orders
    LEFT JOIN OrderDetail on Orders.orderID = OrderDetail.orderID
    LEFT JOIN MenuItem on MenuItem.foodName = OrderDetail.foodName and MenuItem.foodTruckName = OrderDetail.foodTruckName
    LEFT JOIN Customer on Customer.username = Orders.customerUsername
    where username = i_customerUsername
